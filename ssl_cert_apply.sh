...
# 复制证书文件到nginx目录
cp "/root/.acme.sh/$DOMAIN"_ecc/fullchain.cer "$CERT_PATH/$DOMAIN.pem"
cp "/root/.acme.sh/$DOMAIN"_ecc/"$DOMAIN.key" "$CERT_PATH/$DOMAIN.key"
...
if [ -f "$CERT_PATH/$DOMAIN.pem" ] && [ -f "$CERT_PATH/$DOMAIN.key" ]; then
    ...
    # 添加到crontab，每天运行一次
    CRON_CMD="0 0 * * * $SCRIPT_PATH \"$DOMAIN\""
    (crontab -l 2>/dev/null | grep -v "$SCRIPT_PATH" ; echo "$CRON_CMD") | crontab -
    
    echo "已添加自动更新计划任务，每天检查并自动更新一次证书"
    echo "当前crontab任务："
